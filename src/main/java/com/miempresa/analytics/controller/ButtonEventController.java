...
import com.miempresa.analytics.service.ButtonEventService;
...
@RequestMapping("/api")
@Validated
@Tag(name = "Button Events", description = "API para gestión de eventos de clic en botones")
public class ButtonEventController {

    private final ButtonEventService service;

    // Inyección de dependencias mediante constructor (Spring Boot best practice)
    public ButtonEventController(ButtonEventService service) {
        this.service = service;
    }

    /**
     * Endpoint para registrar un evento de clic en un botón.
     */
    @PostMapping("/events/button")
    @Operation(summary = "Registrar evento de clic", 
               description = "Registra un nuevo evento de clic en un botón")
    @ApiResponses(value = {
    ...
    public ResponseEntity<ApiResponse<Object>> createButtonEvent(
            @Valid @RequestBody ButtonEventRequest request) {
        try {
    ...
    @Operation(summary = "Obtener eventos paginados", 
               description = "Retorna una lista paginada de todos los eventos de botones")
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200", 
    ...
    public ResponseEntity<ApiResponse<PageResponse<ButtonEventResponse>>> getAllEvents(
            @Parameter(description = "Número de página (0-indexed)", example = "0")
            @RequestParam(defaultValue = "0") int page,
            @Parameter(description = "Tamaño de la página", example = "10")
            @RequestParam(defaultValue = "10") int size) {
        try {
            PageResponse<ButtonEventResponse> events = service.getAllEvents(page, size);
    ...
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            ...
            content = @Content(schema = @Schema(implementation = ButtonEventResponse.class))),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "404", 
            description = "Evento no encontrado",
            content = @Content(schema = @Schema(implementation = ApiResponse.class)))
    })
    public ResponseEntity<ApiResponse<ButtonEventResponse>> getEventById(
            @Parameter(description = "ID del evento", example = "1")
            @PathVariable Long id) {
        try {
            Optional<ButtonEventResponse> event = service.getEventById(id);
            if (event.isPresent()) {
    ...
    /**
     * Endpoint para obtener estadísticas mensuales de clicks por botón y tipo.
     */
    @GetMapping("/stats/monthly")
    @Operation(summary = "Obtener estadísticas mensuales", 
               description = "Retorna agregados mensuales de clicks por botón y tipo")
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200", 
            description = "Estadísticas obtenidas correctamente",
            content = @Content(schema = @Schema(implementation = ButtonMonthlyStat.class)))
    })
    public ResponseEntity<ApiResponse<List<ButtonMonthlyStat>>> getMonthlyStats() {
        try {
            List<ButtonMonthlyStat> stats = service.getMonthlyStats();
